...
namespace Accel.HeaterBoard
{
    public partial class TecHeater : HeaterBase
    {
        ...
        {
            //logger.Debug("E: TecHeater(): construct {0}:{1:d2}", board.Name, id);
        ...
        {
            logger.Debug("I: Device: {0}, Controlling returning: {1}", Name, (stat.Is0Controlling || stat.Is1Controlling).ToString());
            return stat.Is0Controlling || stat.Is1Controlling;
        }
        override internal bool Transitioning(Status stat)
        {
            logger.Debug("I: Device: {0}, Transitioning returning: {1}", Name, (stat.Is0Transitioning || stat.Is1Transitioning).ToString());
        ...
        override internal void Poll()
        {
            ...
            Issue(fCmd(TGetPower,
                    (temp) => { _lastPower = temp; NotifyPropertyChange("PowerLevel"); }), true);
            Issue(fCmd(TGetCurrent,
                    (temp) => { _lastCurrent = temp; NotifyPropertyChange("Current"); }), true);
        }
