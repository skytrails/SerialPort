...
namespace Accel.HeaterBoard
{
    ...
    // wraps the status bytes in a response
    {
        ...
        public bool Is0Transitioning
        {
            get
            {
                return (_status & Transition_0) != 0;
            }
        }
        public bool Is0Controlling
        {
            get
            {
                return (_status & Control_0) != 0;
            }
        }
    ...
    {
        internal const int MinimumHeaterPollInterval = 500;
        internal const int MinimumFanPollInterval = 500;
        ...
        {
            logger.Debug("I: {0} BoardDevice.Initialize()", Name);
            ReadFirmwareVersion();
            if (config != null)
                LoadParams(config);       // reads values from config, sets params in controller
            logger.Debug("O: BoardDevice.Initialize()");
        }
        ...
        {
            Issue(sCmd(GetFirmwareVer, "0", (ver) => { _firmware = ver; }), true);
        }
        ...
        public bool GetStatus(int channel, ref bool bControlling0, ref bool bControlling1,
            ref bool bTransitioning0, ref bool bTransitioning1)
        {
            bool bRetVal = true;

            bool bLocalControlling0 = false;
            bool bLocalControlling1 = false;
            bool bLocalTransitioning0 = false;
            bool bLocalTransitioning1 = false;

            Issue(sCmd(TGetStatus, channel.ToString("00"), (status) =>
            {
                Logger logger = LogManager.GetCurrentClassLogger();

                int nStatus = int.Parse(status);

                logger.Debug("GetStatus: status is {0}", status);
                Console.WriteLine(string.Format("GetStatus: status is {0}", status));

                bLocalControlling0 = (nStatus & 0x0010) != 0;
                bLocalControlling1 = (nStatus & 0x0060) != 0;
                bLocalTransitioning0 = (nStatus & 0x0008) != 0;
                bLocalTransitioning1 = (nStatus & 0x0020) != 0;

                logger.Debug("GetStatus: Controlling0: {0} Controlling1: {1} Transitioning0: {2} Transitioning1: {3}",
                    bLocalControlling0.ToString(),
                    bLocalControlling1.ToString(),
                    bLocalTransitioning0.ToString(),
                    bLocalTransitioning1.ToString()
                    );
                Console.WriteLine(string.Format("GetStatus: Controlling0: {0} Controlling1: {1} Transitioning0: {2} Transitioning1: {3}",
                     bLocalControlling0.ToString(),
                     bLocalControlling1.ToString(),
                     bLocalTransitioning0.ToString(),
                     bLocalTransitioning1.ToString()
            ...
            bControlling0 = bLocalControlling0;
            bControlling1 = bLocalControlling1;
            bTransitioning0 = bLocalTransitioning0;
            bTransitioning1 = bLocalTransitioning1;
        ...
        {
            //logger.Debug("E: {0} BoardDevice.Poll()", Name);
        }
        ...
        internal void Issue(Command cmd, bool priority = false)
        {
            logger.Debug("IssueAndListen, sending command; {0}", XToString(cmd.Text()));
            _board.Issue(cmd, priority);
        }
    ...
    public partial class Board : INamed, IDisposable
    {
        ...
        public Board(string name, TransportBase tport = null)
        {
            ...
            logger.Debug("I: Board(): construct {0}{1}", name, (tport == null ? "" : " with supplied transport"));
            Name = name;
            _transport = tport;
        ...
        public void Initialize(ConfigurationElement config)
        {
            ...
            logger.Debug("I: Board.Initialize() {0} with config for {1} TECs, {2} RHs", Name, cfg.Tecs.Count, cfg.ResHtrs.Count);
            if (_transport == null)
            {
            ...
            // Query a processor (just one, although there are eight) for board-specific data
            // (Each processor should have the same information.)
            Issue(new sCommand(BoardProxyAddr + BoardDevice.BoardRev + "0", OnProxyStatus, (rev) => { _board_rev = rev; }), true);
            Issue(new sCommand(BoardProxyAddr + BoardDevice.BoardSerNo + "0", OnProxyStatus, (rev) => { _serial_no = rev; }), true);
            DeactivateAll(false);
            ...
            logger.Debug("O: Board.Initialize()");
        }
        ...
        internal void DeactivateAll(bool block)
        {
            ...
            Issue(new vCommand("07MD0=0", (st) => true, on_last_message), true);
