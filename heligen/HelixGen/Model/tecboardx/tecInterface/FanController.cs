...
namespace Accel.HeaterBoard
{
    ...
    public class FanController : BoardDevice
    {
        ...
        {
            logger.Trace("E: FanController(): construct {0}:{1:d2}", board.Name, id);
            _fans = new List<Fan>();
        ...
        override public void Initialize(ConfigurationElement config)
        {
            ...
            Issue(iCmd(FGetAvailableThermos, (mask) => SetAvailableThermistors(mask)));
        }
        ...
        public int FanDutyCycle
        {
            ...
            set
            {
                ...
                logger.Trace("E: {0} FanDutyCycle <= {1}", Name, value);
                Issue(iCmd(FPWM, DefaultChannel(), value.ToString(), (ival) => { _dutycycle = value; }));
            }
        }
        ...
        {
            logger.Trace("E: {0} SetPoint <= {1}", Name, active);
            Issue(iCmd(command, DefaultChannel(), ActivationValue(active), handler));
        }
