...
namespace System_Protocol_File_Reader
{
    ...
    {
        public CTestProcess_Protocol_PressureThreshold(int lineNo, int threshold = -1, string messageIn = "") : base(lineNo, eTestProcess_Statement_Type.kPressureThreshold)
        {
        ...
        /// <summary>
        /// The message to be displayed if the threshold is exceeded.
        /// </summary>
        public string message
        {
            get { return _message; }
    ...
    {
        public CTestProcess_Protocol_SetPressure(int lineNo, int nPressureIn) : base(lineNo, eTestProcess_Statement_Type.kSetPressure)
        {
            _nPressure = nPressureIn;
        }

        protected int _nPressure;

        public int pressure
        {
            get { return _nPressure; }
            set { _nPressure = value; }
        }
    }
    ...
    public class CPCR_ProtocolBaseStepDefn
    {
        ...
        virtual public int                          m_Pressure { get; set; }    // Optional pressure setting. -1 if not set.
        virtual public string                       m_valveName { get; set; }   // Optional valve setting, blank if not set.
        virtual public bool                         m_valveOpen { get; set; }   // Optional valve setting, ignored if m_valveName is blank.
    ...
    {
        public CSystem_Protocol_File_Reader(CSystem_Configuration_File_Reader.CSystem_Configuration_Obj configurationObj)
        {
            _m_configurationObj = configurationObj;
        }

        private CSystem_Configuration_File_Reader.CSystem_Configuration_Obj _m_configurationObj;
                ...
                if (statement_node.NodeType == XmlNodeType.Element)
                {
                    switch (statement_node.Name.ToUpper())
                    {
                        ...
                        case "PRESSURETHRESHOLD":
                            {
                                ...
                                bSuccess = Int32.TryParse(args[0], out nThreshold);

                                if (!bSuccess)
                                {
                                    throw new Exception(string.Format("Protocol Parse Error. Test section, the threshold argument \"{0}\" for the {1} statement on line: {2} must be a valid positive integer.",
                                        args[0],
                                         statement_node.Name,
                                         statement_node.LineNumber.ToString()));
                                }

                                if (bSuccess && nThreshold < 0)
                                {
                                    throw new Exception(string.Format("Protocol Parse Error. Test section, the threshold argument \"{0}\" for the {1} statement on line: {2} must be a positive integer.",
                                        args[0],
                                         statement_node.Name,
                                         statement_node.LineNumber.ToString()));
                                }
                                ...
                                test_protocolIn.m_TestProcess_Statements.Add(
                                    new CTestProcess_Protocol_PressureThreshold(statement_node.LineNumber, nThreshold, args[1])
                                    );
                            }
                            break;
                        ...
                        case "SETPRESSURE":
                            ...
                            bSuccess = Int32.TryParse(statement_node.InnerText.Trim(), out nPressure);

                            if (!bSuccess)
                            {
                                throw new Exception(string.Format("Protocol Parse Error. Test section, the pressure argument \"{0}\" for the {1} statement on line: {2} must be a valid positive integer.",
                                    statement_node.InnerText,
                                     statement_node.Name,
                                     statement_node.LineNumber.ToString()));
                            }

                            test_protocolIn.m_TestProcess_Statements.Add(new CTestProcess_Protocol_SetPressure(statement_node.LineNumber, nPressure));
                        ...
                        case "VALVE":
                            {
                                ...
                                test_protocolIn.m_TestProcess_Statements.Add(new CTestProcess_Protocol_Valve(statement_node.LineNumber, args[0], (args[1] == "OPEN")));
                            }
...
#pragma warning restore
        ...
        private void ParseProtocolPneumaticProcessStatements(Positional_XmlElement pneumaticprocess_protocol_statement_nodes, CPneumaticProcess_Protocol_Process_Obj pneumatic_protocol_obj, out string strChipType)
        {
            ...
            foreach (Positional_XmlElement statement_node in pneumaticprocess_protocol_statement_nodes)
            {
                if (statement_node.NodeType == XmlNodeType.Element)
                {
                    if ("CHIPTYPE" == statement_node.Name.ToUpper())
                    {
                    ...
                    else if ("NAME" == statement_node.Name.ToUpper())
                    {
                    ...
                    else if ("LOAD" == statement_node.Name.ToUpper())
                    {
                    ...
                    else if ("PRIME" == statement_node.Name.ToUpper())
                    {
                    ...
                    else if ("DIGITIZE" == statement_node.Name.ToUpper())
                    {
                    ...
                    else if ("PCR_PROCESS" == statement_node.Name.ToUpper())
                    {
                        ...
                        foreach (Positional_XmlElement step_subnode in step_node)
                        {
                            if (step_subnode.NodeType == XmlNodeType.Element)
                            {
                                    ...
                                    if (!bOptionalMeasurementFound)
                                    {
                                        ...
                                        foreach (Positional_XmlElement measurement_subnode in step_subnode)
                                        {
                                            if (measurement_subnode.NodeType == XmlNodeType.Element)
                                            {
                                                ...
                                                if ("EXPOSURETIMEINMICROSECONDS" == subtag_name)
                                                {
                                                ...
                                                else if ("LEDINTENSITY" == subtag_name)
                                                {
                                                    int nIntensity = 0;
                                ...
                                else if ("VALVE" == tag_name)
                                {
                                    ...
                                    StepDefn.m_valveName = args[0];
                                    StepDefn.m_valveOpen = args[1] == "OPEN";
                                }
                                else if ("PRESSURE" == tag_name)
                                {
                                    int nPressure;
                                    bool bSuccess = Int32.TryParse(step_subnode.InnerText.Trim(), out nPressure);
        ...
        private void ParseProtocolPCRCycleDefinitions(Positional_XmlElement cycle_nodes, CSystem_Protocol_PCR_Process_Obj protocol_obj)
        {
            ...
            foreach (Positional_XmlElement cycle_node in cycle_nodes)
            {
                ...
                {
                    if ("Cycle".ToUpper() == cycle_node.Name.ToUpper())
                    {
                        bool bRequiredNameFound = false;
                                ...
                                else if ("OPERATIONS" == tag_name)
                                {
                                    ...
                                    foreach (Positional_XmlElement operation_node in cycle_subnode)
                                    {
                                        if (operation_node.NodeType == XmlNodeType.Element)
                                        {
                                            ...
                                            else if ("CYCLE" == cycle_entry_tag_name)
                                            {
                                                if (iOperationOrderIndex > 0)
                                                {
                                                    Array.Resize<CPCR_OperationDescriptor>(ref CycleStep.m_Operations, iOperationOrderIndex + 1);
            ...
            if ("PROCESS_SEQUENCE" == process_sequence_node.Name.ToUpper())
            {
                ...
                foreach (Positional_XmlElement process_sequence_subnode in process_sequence_node)
                {
                    if (process_sequence_subnode.NodeType == XmlNodeType.Element)
                    {
                        ...
                        else if ("LOG_RATE_IN_SECS" == tag_name)
                        {
                        ...
                        else if ("PROCESS_STEPS" == tag_name)
                        {
                            ...
                            foreach (Positional_XmlElement operation_node in process_sequence_subnode)
                            {
                                if (operation_node.NodeType == XmlNodeType.Element)
                                {
                                    ...
                                    else if ("CYCLE" == operation_tag_name)
                                    {
                                        if (iOperationOrderIndex > 0)
                                        {
                                            Array.Resize<CPCR_OperationDescriptor>(ref protocol_obj.m_PCR_ProtocolProcess.m_OperationSequence, iOperationOrderIndex + 1);
        ...
        private void ProtocolPCRSemanticValidation(CSystem_Protocol_PCR_Process_Obj protocol_obj, out string strWarning)
        {
            ...
            foreach (CPCR_OperationDescriptor process_step_entry in protocol_obj.m_PCR_ProtocolProcess.m_OperationSequence)
            {
                if (process_step_entry.m_OpType == ePCR_ProtocolOperationType.kPCR_Step)
                {
                    ...
                    {
                        step.m_fTemperatureinC = _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolNonCycleStep_MinTemperature;
                        ...
                        if (step.m_fHoldTimeinSecs > _m_configurationObj.m_MiscellaneousConfiguration.m_uiProtocolNonCycleStep_MinTemp_MaxHoldTimeinSeconds)
                        {
                            step.m_fHoldTimeinSecs = _m_configurationObj.m_MiscellaneousConfiguration.m_uiProtocolNonCycleStep_MinTemp_MaxHoldTimeinSeconds;
                    ...
                    if (step.m_fTemperatureinC > _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolNonCycleStep_MaxTemperature)
                    {
                        step.m_fTemperatureinC = _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolNonCycleStep_MaxTemperature;
                        protocol_obj.m_PCR_StepDefns[process_step_entry.m_strOperationName] = step; // Replace with updated step definition
                            ...
                            case ePCR_ProtocolOperationType.kPCR_Step:
                                {
                                    ...
                                    if (step.m_fTemperatureinC < _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolCycleStep_MinTemperature)
                                    {
                                         step.m_fTemperatureinC = _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolCycleStep_MinTemperature;
                                    ...
                                    if (step.m_fTemperatureinC > _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolCycleStep_MaxTemperature)
                                    {
                                        step.m_fTemperatureinC = _m_configurationObj.m_MiscellaneousConfiguration.m_fProtocolCycleStep_MaxTemperature;
                                        protocol_obj.m_PCR_StepDefns[cycle_operation_name] = step; // Replace with clamped value
